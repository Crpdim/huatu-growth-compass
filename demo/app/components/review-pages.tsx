...
export function ReviewPages(props: ReviewPagesProps) {
  ...
  if (stage === "companionReview") {
    return <section className="content-page companion-review-page">
      <div className="companion-review-heading"><div><span className="section-kicker">周期复盘与画像校准</span><h2>每一次完成，<br />都让画像更接近真实的你</h2></div><p>能力变化来自任务和成果证据。一次聊天不会突然改变你，画像会留下每次更新的理由。</p></div>
      ...
      {reviewWindow !== "week" ? <article className="review-window-placeholder"><div><span>{reviewWindowData[reviewWindow].metric}</span><small>{reviewWindow === "day" ? "今日任务" : reviewWindow === "month" ? "已积累" : "当前画像"}</small></div><section><span>能力预览</span><h3>{reviewWindowData[reviewWindow].title}</h3><p>{reviewWindowData[reviewWindow].description}</p><small>{reviewWindowData[reviewWindow].note}</small></section><button className="primary-button" onClick={() => onSetReviewWindow("week")}>查看完整周复盘 <span>→</span></button></article> : <>
        <div className="companion-review-metrics"><article><span>加权完成率</span><strong>60%</strong><small>3 项任务完成</small></article><article><span>目标偏离度</span><strong>20%</strong><small>申论体验待完成</small></article><article><span>本周投入</span><strong>58<small>min</small></strong><small>岗位、课程与练习</small></article><article><span>新增证据</span><strong>{evidenceWithdrawn ? 0 : 1}<small>条</small></strong><small>{evidenceWithdrawn ? "已由用户撤回" : "课程结果与反馈"}</small></article><article><span>节点风险</span><strong>0<small>项</small></strong><small>暂无紧急节点</small></article></div>
        <div className="profile-review-grid">
          ...
          <aside className="dynamic-plan-review"><header><span className="agent-avatar">AI</span><div><small>周复盘结论</small><h3>下一周，调整学习重点</h3></div></header><div className="weekly-review-summary"><span>本周判断</span><p>你已经完成第一次真实学习。{learningFeedbackOutcome}</p></div><div className="plan-change-list"><div><span>资料分析</span><s>完成一套综合练习</s><i>→</i><b>{learningFeedbackNextTask}</b><small>根据本轮学习反馈调整</small></div><div><span>申论体验</span><s>完成 2 篇阅读</s><i>→</i><b>先保留 1 篇，另一篇顺延</b><small>控制总任务量</small></div><div><span>每日投入</span><s>60 分钟</s><i>→</i><b>工作日 30 分钟</b><small>与课程作业协调</small></div></div><div className="review-evidence-note"><span>{evidenceWithdrawn ? "画像未更新" : "本轮新增 1 条证据"}</span><b>{evidenceWithdrawn ? "用户已撤回课程结果记录" : `完成《${activeResourceTitle}》与随堂反馈`}</b><small>每项计划变化都能回到证据来源。</small></div><button className="primary-button full" onClick={() => onGoToStage("stageSummary")}>进入阶段结算 <span>→</span></button></aside>
        </div>
      </>}
    </section>;
  }
