    ...
    return (
      <section className="content-page profile-page">
        <div className="page-heading split-heading">
          ...
          <p>填写基本信息，也可以授权常用平台。AI 只读取你勾选的范围，用来判断基础、习惯和可投入时间。</p>
        </div>
      ...
      <section className="content-page analysis-page">
        ...
        <div className="analysis-workspace">
          <article className="analysis-engine-card">
            ...
            <div className="analysis-sources"><span>情境回答 {answeredCount} 条</span><span>手动资料 5 类</span>{selectedAuthorizationSources.length > 0 ? selectedAuthorizationSources.map((source) => <span key={source.id}>{source.name}</span>) : <span>未授权外部平台</span>}</div>
          </article>
          <article className="analysis-trace-card">
            ...
            <div className={`analysis-result ${analysisComplete ? "is-visible" : ""}`}><div><span>生成结果</span><b>六维能力与任务画像</b></div><div><span>路径线索</span><b>考公方向更突出</b></div><div><span>行动偏好</span><b>短任务、周复盘</b></div></div>
