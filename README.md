...
> 成长罗盘：人生课题 → 情境探索 → 初始画像 → 三条路径与同路人案例 → 完成方向探索 → 导入画像 → 成长管理：Todo 执行 → AI 伴学 → 华图课程 → 周期复盘与能力雷达校准 → 阶段结算 → 下一阶段分流

产品逻辑支持考研、公考/考编和企业求职三条路径，比赛 Demo 使用考公完整演示一段阶段性规划，另外两条作为对照。首版聚焦可授权画像补全、推荐依据、任务管理、华图资源伴学、证据更新和动态校准。AI 服务异常时，静态内容仍能支撑完整演示。
