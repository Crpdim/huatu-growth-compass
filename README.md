...
产品逻辑支持考研、公考/考编和企业求职三条路径，比赛 Demo 使用考公完整演示一段阶段性规划，另外两条作为对照。八维画像将性格适配、专业能力、兴趣匹配度、学习与知识、抗压与情绪、沟通协作、健康情况和家庭情况统一放进八轴雷达；界面只展示面向用户的结论、依据和置信度，不展示内部信号、权重公式或提示词。成长管理以 Agent 会话为主入口，并保留一个与对话共享状态的独立任务管理页；学生可以直接新增、编辑、暂停或删除任务，也可以从单个任务旁打开 AI 助手，演示任务拆解和官方网址检索。周回顾和阶段总结继续在会话内呈现。AI 服务异常时，静态内容仍能支撑完整演示。
