...
> 人生模拟器 → 模范公务员结局 → 补全任务画像 → 八维雷达画像 → 三条路径与同路人案例 → 导入成长管理 → Agent 会话与独立任务管理中的执行、查询和动态校准

Demo 首页是“模拟人生”小游戏。用户可以从右上角“没有人生目标？”直接进入原有方向探索，也可以完成体制内人生并达成“模范公务员”，再通过“补全任务画像”进入真实信息补充。小游戏当前完整演示体制内人生，其余人生路线保留为静态入口。

产品逻辑支持考研、公考/考编和企业求职三条路径，比赛 Demo 使用考公完整演示一段阶段性规划，另外两条作为对照。八维画像将性格适配、专业能力、兴趣匹配度、学习与知识、抗压与情绪、沟通协作、健康情况和家庭情况统一放进八轴雷达；界面只展示面向用户的结论、依据和置信度，不展示内部信号、权重公式或提示词。成长管理以 Agent 会话为主入口，并保留一个与对话共享状态的独立任务管理页；周回顾和阶段总结继续在会话内呈现。AI 服务异常时，静态内容仍能支撑完整演示。
